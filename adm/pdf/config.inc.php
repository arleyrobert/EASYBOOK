                        ...
                        private function connect(){
                    		$this->db_connect_id = mysqli_connect($this->obj['dbhost'],$this->obj['dbuser'],$this->obj['dbpwd'],$this->obj['dbname']);
                                 if (!$this->db_connect_id)
                                  {
                                    echo (" Erro! nao foi possivel conectar ao servidor:".mysqli_connect_error());
                        	  }
                      }
...
function execute($query) {       
        ...
        if(!$this->q_id ) {
            ...
            die ("ERROR: error DB.<br> Não foi possível executar a consulta:<br> $query <br>MySql Tipo De Erro: $error1");
            exit;
        }         
	$this->query_count++; 
	return $this->q_id;    
